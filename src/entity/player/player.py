...
class Player:
    ...
    def render(self, screen, camera):
        ...
        self.left_arm_member.render(screen, 0, 0, camera, utils.map(math.cos((utils.get_current_time_millis() - self.start_at) / 300), -1, 1, -3, 2))
        self.right_arm_member.render(screen, 0, 0, camera, utils.map(math.cos((utils.get_current_time_millis() - self.start_at) / 350), -1, 1, -3, 2))
